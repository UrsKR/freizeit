...
class FeiertagServiceTests extends GrailsUnitTestCase {
    ...
    void testTreatsSameStartAndEndAsOneDay() {
        def start = getDate(2011, Calendar.MARCH, 04)
        def end = getDate(2011, Calendar.MARCH, 04)
        assertWorkdays(start..end, 1)
    }

      void testTreatsLongWeekendAsTwoDays() {
        def start = getDate(2011, Calendar.MARCH, 04)
        def end = getDate(2011, Calendar.MARCH, 07)
        assertWorkdays(start..end, 2)
    }
